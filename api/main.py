    ...
    for row in rows:
        ...
        results.append(
            FeeComparisonResult(
                ...
                fee_amount=float(row["fee_amount"]) if row.get("fee_amount") is not None else None,
                fee_percent=float(row["fee_percent"]) if row.get("fee_percent") is not None else None,
